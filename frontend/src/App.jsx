...
function Settings() {
  ...
  // Paid plans are not live yet. The pricing table stays visible so the plans
    ...
    const TABS=[{k:'company',i:'&#127962;',l:'Company'},{k:'bank',i:'&#127974;',l:'Bank & GST'},{k:'pricing',i:'&#128176;',l:'Pricing & Terms'},{k:'pdf',i:'&#127912;',l:'PDF'},{k:'wa',i:'&#128172;',l:'WhatsApp'},{k:'users',i:'&#128101;',l:'Users'},{k:'plan',i:'&#9889;',l:'Plan'}]
  ...
  return(
    <div style={{display:'flex',gap:20,maxWidth:1100}}>
      ...
      <div style={{flex:1,minWidth:0}}>
        ...
        {tab==='pricing'&&(<div style={{background:C.snow,borderRadius:16,border:'1px solid '+C.glass,padding:24}}>
          <div style={{fontFamily:'Syne,sans-serif',fontSize:16,fontWeight:700,marginBottom:4}}>Pricing &amp; Terms</div>
          <div style={{fontSize:12,color:C.mist,marginBottom:20}}>Applied to every new quote. Rates per sqft are added on top of material cost.</div>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0 20px'}}>
            <div>
              <label style={lb}>Wastage %</label>
              <input type="number" step="0.5" value={co?.wastage_percent ?? 5} onChange={e=>upd('wastage_percent',Number(e.target.value)||0)} style={si}/>
              <div style={{fontSize:11,color:C.mist,marginTop:-12,marginBottom:14}}>Offcuts and cutting loss, added to material cost.</div>
              <label style={lb}>Labour per sqft (&#8377;)</label>
              <input type="number" step="1" value={co?.labour_per_sqft ?? 0} onChange={e=>upd('labour_per_sqft',Number(e.target.value)||0)} style={si}/>
            </div>
            <div>
              <label style={lb}>Installation per sqft (&#8377;)</label>
              <input type="number" step="1" value={co?.installation_per_sqft ?? 0} onChange={e=>upd('installation_per_sqft',Number(e.target.value)||0)} style={si}/>
              <div style={{fontSize:11,color:C.mist,marginTop:-12,marginBottom:14}}>Suggested installation charge. You can still override it per quote.</div>
              <label style={lb}>State code</label>
              <input value={co?.state_code||''} onChange={e=>upd('state_code',e.target.value)} placeholder="e.g. 29 for Karnataka" style={si}/>
            </div>
          </div>
          <div style={{borderTop:'1px solid '+C.glass,marginTop:8,paddingTop:20}}>
            <label style={lb}>Quotation terms</label>
            <textarea value={co?.terms_quotation||''} onChange={e=>upd('terms_quotation',e.target.value)}
              placeholder={'1. Payment: 50% advance with order, balance before delivery.\n2. Quote valid 15 days.\n3. Prices hold for size variation up to +/- 25mm.\n4. PAN required for orders above Rs 2 lakh.'}
              style={{...si,minHeight:130,resize:'vertical',fontFamily:'Inter,sans-serif',lineHeight:1.6}}/>
            <label style={lb}>Invoice terms</label>
            <textarea value={co?.terms_billing||''} onChange={e=>upd('terms_billing',e.target.value)}
              placeholder={'1. Goods once sold will not be taken back.\n2. Interest at 18% p.a. on overdue amounts.\n3. Subject to local jurisdiction.'}
              style={{...si,minHeight:110,resize:'vertical',fontFamily:'Inter,sans-serif',lineHeight:1.6}}/>
          </div>
          <div style={{textAlign:'right'}}>
            <button style={sb} onClick={save} disabled={saving}>{saving?'\u23f3 Saving...':'\u2713 Save'}</button>
          </div>
        </div>)}
